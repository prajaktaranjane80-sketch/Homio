...
def test_safe_resume_is_not_blind_continue() -> None:
    report = validate_resume_safety(make_request())

    assert report.validated is True
    assert report.decision == ResumeDecision.SAFE_TO_RESUME
...
def test_missing_checkpoint_blocks_resume() -> None:
    report = validate_resume_safety(
        make_request(checkpoint_available=False)
    )

    assert report.decision == ResumeDecision.BLOCK_RESUME
    assert report.reason == ResumeSafetyReason.MISSING_CHECKPOINT
    assert is_safe_to_resume(report) is False


def test_invalid_checkpoint_fails_closed() -> None:
    report = validate_resume_safety(
        make_request(checkpoint_valid=False)
    )

    assert report.decision == ResumeDecision.FAIL_CLOSED
    assert report.reason == ResumeSafetyReason.CHECKPOINT_INVALID
    assert report.fail_closed is True


def test_unavailable_state_blocks_resume() -> None:
    report = validate_resume_safety(
        make_request(state_available=False)
    )
...
def test_invalid_state_fails_closed() -> None:
    report = validate_resume_safety(
        make_request(state_valid=False)
    )
...
def test_invalid_gate_fails_closed() -> None:
    report = validate_resume_safety(
        make_request(gate_valid=False)
    )

    assert report.decision == ResumeDecision.FAIL_CLOSED
    assert report.reason == ResumeSafetyReason.GATE_INVALID


def test_unclear_authority_is_not_accepted() -> None:
    with pytest.raises(ResumeSafetyAuthorityError):
        validate_resume_safety(
            make_request(authority_valid=False)
        )


def test_integrity_failure_is_not_accepted() -> None:
    with pytest.raises(ResumeSafetyIntegrityError):
        validate_resume_safety(
            make_request(integrity_valid=False)
        )


def test_architecture_drift_fails_closed() -> None:
    report = validate_resume_safety(
        make_request(architecture_stable=False)
    )

    assert report.decision == ResumeDecision.FAIL_CLOSED
    assert report.reason == ResumeSafetyReason.ARCHITECTURE_DRIFT


def test_ambiguous_state_fails_closed() -> None:
    report = validate_resume_safety(
        make_request(state_ambiguous=True)
    )

    assert report.decision == ResumeDecision.FAIL_CLOSED
    assert report.reason == ResumeSafetyReason.AMBIGUOUS_STATE


def test_stale_state_blocks_resume() -> None:
    report = validate_resume_safety(
        make_request(state_stale=True)
    )

    assert report.decision == ResumeDecision.BLOCK_RESUME
    assert report.reason == ResumeSafetyReason.STALE_STATE


def test_unsafe_recovery_blocks_resume() -> None:
    report = validate_resume_safety(
        make_request(recovery_safe=False)
    )

    assert report.decision == ResumeDecision.BLOCK_RESUME
    assert report.reason == ResumeSafetyReason.RECOVERY_UNSAFE
