...
T01 provides a read-only, deterministic Project DNA projection for a
new AI/operator session.

IMPORTANT AUTHORITY RULE
------------------------
This module does not create a second project state.
...
PROJECT_CONTINUITY remains a continuity contract and does not replace
controller state.

ACRL only reconstructs and exposes a compact identity projection.
...
class ProjectDNASourceError(ProjectDNAError):
    """Raised when an authoritative DNA source is missing or invalid."""
...
class ProjectDNA:
    ...
    def resume_identity(self) -> str:
        """Return the compact identity required for session reconstruction."""

        return "\n".join(
            (
                "HOMIO / REOS — PROJECT DNA v1.0",
                "=" * 58,
