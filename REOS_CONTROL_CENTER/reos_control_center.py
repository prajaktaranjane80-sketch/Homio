...
def load_state() -> dict[str, Any]:
    """
    Load and integrity-check the canonical REOS Control Center state.
...
def load_templates() -> dict[str, Any]:
    ...
    if TEMPLATES.exists():
        try:
            ...
            raise SystemExit(
                f"Invalid GATE_TEMPLATES.json: {exc}"
            ) from exc
...
def materialize_current_gate(
    ...
    if not template:
        raise SystemExit(
            f"No template available for current gate: {gate_id}"
        )
    ...
    gate = {
        ...
        "subtasks": [
            ...
            for index, (sid, title, priority)
            in enumerate(subtasks)
        ],
        ...
        "criteria_state": [
            ...
            for index, criterion
            in enumerate(template["criteria"], start=1)
        ],
        "completion_policy": (
            "All blocking subtasks DONE + all criteria "
            "VERIFIED + explicit approval."
        ),
        "created_at": now(),
    }
...
def build_handoff_packet(
    ...
    lines.extend(
        f"{x['id']}|{x['status']}|{x['title']}"
        for x in gate["subtasks"]
    )
    ...
    lines.extend(
        f"{x['id']}|{x['status']}|{x['criterion']}"
        for x in gate["criteria_state"]
    )
    ...
    (ROOT / "REOS_NEXT_CHAT.txt").write_text(
        packet,
        encoding="utf-8",
    )
...
def cmd_status(state: dict[str, Any]) -> None:
    ...
    print(
        "Phase:",
        state.get("phases", {}).get("current", "UNKNOWN"),
    )
    print("Gate:", execution.get("current_gate"))
    print("Task:", execution.get("current_task"))
    print("Status:", execution.get("status"))
    print("Subtask:", gate.get("current_subtask"))
    print("Gate Status:", gate.get("status"))
    print(
        "Subtasks:",
        len(gate.get("subtasks", [])),
        "Criteria:",
        len(gate.get("criteria_state", [])),
    )
    print(
        "Approved:",
        len(state.get("architecture", {}).get("approved", [])),
        "Pending:",
        len(state.get("architecture", {}).get("pending", [])),
    )
    print("State:", STATE)
...
def cmd_complete_subtask(
    ...
    if not subtask:
        raise SystemExit(
            f"Subtask not found: {subtask_id}"
        )

    if subtask["status"] != "CURRENT":
        raise SystemExit(
            "Only CURRENT subtask can be completed. "
            f"{subtask_id} is {subtask['status']}"
        )

    if is_approval_subtask(subtask):
        raise SystemExit(
            "Approval/freeze subtask is controlled by "
            "approve-gate, not complete-subtask."
        )
    ...
    if remaining:
        ...
        state["execution"]["current_subtask"] = (
            remaining[0]["id"]
        )
        ...
        state["execution"]["current_subtask"] = next(
            (
                x["id"]
                for x in gate["subtasks"]
                if is_approval_subtask(x)
            ),
            None,
        )

        state["execution"]["current_task"] = (
            f"{current_gate_id(state)}: "
...
def cmd_verify_criterion(
    ...
    if not criterion:
        raise SystemExit(
            f"Criterion not found: {criterion_id}"
        )
...
def cmd_verify_all(state: dict[str, Any]) -> None:
    ...
    evidence = (
        "Final review completed by current gate execution."
    )
    ...
    print(
        "VERIFIED CRITERIA:",
        len(gate["criteria_state"]),
    )


def cmd_validate_gate(
    state: dict[str, Any],
) -> bool:
    gate = materialize_current_gate(state)

    approval_subtask = next(
        (
            x
            for x in gate["subtasks"]
            if is_approval_subtask(x)
    ...
    unfinished = [
        x["id"]
        for x in gate["subtasks"]
        if x is not approval_subtask
        and x["status"] not in ("DONE", "CLOSED")
    ]
    ...
    if approval_subtask:
        approval_subtask["status"] = (
            "READY_FOR_APPROVAL"
        )

        gate["current_subtask"] = (
            approval_subtask["id"]
        )
    ...
    print(
        "GATE VALIDATED:",
        current_gate_id(state),
    )

    print(
        "APPROVAL READY: "
        "run python reos_control_center.py approve-gate"
    )

    return True


def cmd_approve_gate(
    state: dict[str, Any],
) -> None:
    gate = materialize_current_gate(state)
    ...
    if gate.get("status") != "VALIDATED":
        raise SystemExit(
            "Gate must be VALIDATED before approval. "
            "Run: python reos_control_center.py validate-gate"
        )
    ...
    pending_item = next(
        (
            x
            for x in state["architecture"]["pending"]
            if x["id"] == gid
        ),
        None,
    )
    ...
    if pending_item:
        state["architecture"]["pending"].remove(
            pending_item
        )
        ...
        state["architecture"]["approved"].append(
            pending_item
        )
    ...
    for subtask in gate["subtasks"]:
        if (
            subtask["status"]
            in ("CURRENT", "READY_FOR_APPROVAL")
            or is_approval_subtask(subtask)
        ):
            subtask["status"] = "DONE"
            subtask["completed_at"] = now()
    ...
    if current_index is None:
        raise SystemExit(
            f"Current gate {gid} is missing from execution_plan"
        )

    sequence[current_index]["status"] = "COMPLETE"

    next_gate = (
        sequence[current_index + 1]["gate"]
    ...
    if next_gate:
        ...
        state["execution"]["current_task"] = next(
            x["name"]
            for x in sequence
            if x["gate"] == next_gate
        )
        ...
        state["execution"]["current_subtask"] = (
            next_plan["current_subtask"]
        )

        state["execution"]["current_task"] = (
            f"{next_plan['current_subtask']}: "
            f"{next_plan['subtasks'][0]['title']}."
        )
        ...
        state["execution"]["current_subtask"] = None
        state["execution"]["current_task"] = (
            "All authoritative gates complete."
        )

    emit(
        state,
        "GATE_APPROVED_AND_FROZEN",
        {
            "gate": gid,
            "next_gate": next_gate,
        },
...
def cmd_verify_state(
    ...
    print(
        "STATE HASH CALCULATED:",
        calculated,
    )
    print(
        "INTEGRITY:",
        "PASS" if stored == calculated else "MISMATCH",
    )


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="reos_control_center.py"
    )
